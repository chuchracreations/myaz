import React, { useMemo, useState } from 'react';
import { Search, X, Copy, Check } from 'lucide-react';

export interface CheatCommand {
  keys: string;
  desc: string;
}

export interface CheatCategory {
  id: string;
  title: string;
  commands: CheatCommand[];
}

interface CheatSheetViewProps {
  icon: React.ReactNode;
  title: string;
  categories: CheatCategory[];
  /** Sets the --cheat-accent custom properties for this guide (see index.css). */
  accentClass: string;
  searchPlaceholder: string;
}

export const CheatSheetView: React.FC<CheatSheetViewProps> = ({
  icon,
  title,
  categories,
  accentClass,
  searchPlaceholder,
}) => {
  const [searchQuery, setSearchQuery] = useState('');
  const [activeCategory, setActiveCategory] = useState('all');
  const [copiedKeys, setCopiedKeys] = useState<string | null>(null);

  const totalCount = useMemo(
    () => categories.reduce((sum, cat) => sum + cat.commands.length, 0),
    [categories]
  );

  const visibleCategories = useMemo(() => {
    const q = searchQuery.trim().toLowerCase();
    return categories
      .map((cat) => ({
        ...cat,
        commands: cat.commands.filter(
          (c) => !q || c.keys.toLowerCase().includes(q) || c.desc.toLowerCase().includes(q)
        ),
      }))
      .filter((cat) => (activeCategory === 'all' || cat.id === activeCategory) && cat.commands.length > 0);
  }, [categories, searchQuery, activeCategory]);

  const handleCopy = (keys: string) => {
    navigator.clipboard.writeText(keys);
    setCopiedKeys(keys);
    setTimeout(() => setCopiedKeys((k) => (k === keys ? null : k)), 1500);
  };

  return (
    <div className="utility-landing">
      <div className="utility-header">
        <div className="utility-icon-tile">{icon}</div>
        <div>
          <div className="utility-title">{title}</div>
          <div className="utility-subtitle">{totalCount} commands, organized for quick lookup</div>
        </div>
      </div>

      <div className="search-container">
        <Search size={13} className="search-icon" />
        <input
          className="search-input"
          type="text"
          placeholder={searchPlaceholder}
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
        />
        {searchQuery && (
          <button
            type="button"
            className="btn-icon"
            style={{ padding: '2px' }}
            onClick={() => setSearchQuery('')}
            title="Clear search"
          >
            <X size={12} />
          </button>
        )}
      </div>

      <div className="filter-pills-row">
        <button
          type="button"
          className={`filter-pill ${activeCategory === 'all' ? 'active' : ''}`}
          onClick={() => setActiveCategory('all')}
        >
          All
        </button>
        {categories.map((cat) => (
          <button
            key={cat.id}
            type="button"
            className={`filter-pill ${activeCategory === cat.id ? 'active' : ''}`}
            onClick={() => setActiveCategory(cat.id)}
          >
            {cat.title}
          </button>
        ))}
      </div>

      {visibleCategories.length === 0 && (
        <div className="empty-state">
          <Search size={22} className="empty-icon" />
          <span className="empty-title">No commands found</span>
          <span className="empty-desc">Try a different search term.</span>
        </div>
      )}

      <div className={`cheat-guide-body ${accentClass}`}>
        {visibleCategories.map((cat) => (
          <div key={cat.id} className="cheat-category-block">
            <span className="discover-label">{cat.title}</span>
            <div className="cheat-command-list">
              {cat.commands.map((cmd) => (
                <button
                  key={cmd.keys}
                  type="button"
                  className="cheat-command-row"
                  onClick={() => handleCopy(cmd.keys)}
                  title="Click to copy"
                >
                  <div className="cheat-command-top-row">
                    <kbd className="cheat-command-keys">{cmd.keys}</kbd>
                    {copiedKeys === cmd.keys ? (
                      <Check size={12} className="cheat-command-copy-icon is-copied" />
                    ) : (
                      <Copy size={12} className="cheat-command-copy-icon" />
                    )}
                  </div>
                  <span className="cheat-command-desc">{cmd.desc}</span>
                </button>
              ))}
            </div>
          </div>
        ))}
      </div>
    </div>
  );
};
